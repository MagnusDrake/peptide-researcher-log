...
export async function exportLogsToCsv(): Promise<string> {
  ...
  const headers = [
    ...
    'Injection Site',
    'Reaction Rating',
    'Custom Reaction Details',
    'Hair, Nails & Skin (1-10)',
    'Food Habit',
    'Body Weight (lbs)',
    'Recovery Score (1-10)',
    'Energy Level (1-10)',
  ...
  const rows = logs.map(log => [
    ...
    `"${(log.customReactionText || '').replace(/"/g, '""')}"`,
    log.subjectiveMetrics?.hairSkinNailsQuality ?? '',
    `"${(log.subjectiveMetrics?.foodHabit || '').replace(/"/g, '""')}"`,
