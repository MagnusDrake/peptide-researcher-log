...
export const LockScreen: React.FC<LockScreenProps> = ({ onUnlock }) => {
  ...
  return (
    <div className={`lock-screen-container fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 sm:p-6 lg:p-10 transition-all duration-[800ms] ease-out ${
      ...
      {/* VIEW 1: FIRST-TIME WELCOME & ONBOARDING PAGE (Shown only when using default PIN) */}
      {isDefaultPin ? (
        ...
        /* VIEW 2: SLEEK MINIMALIST VAULT LOCK SCREEN (Shown once custom PIN is established) */
        <div className={`relative z-10 w-full max-w-sm p-8 flex flex-col items-center transition-all duration-700 ${
          unlocked ? 'translate-y-8 opacity-0' : 'translate-y-0 opacity-100'
        }`}>
          ...
          <div className="grid grid-cols-3 gap-y-6 gap-x-8 w-full max-w-[280px] mx-auto">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
              <button
                ...
                type="button"
                onClick={() => handleKeyPress(num.toString())}
                className="h-16 w-16 mx-auto rounded-full flex items-center justify-center text-2xl font-light text-slate-300 hover:text-cyan-300 hover:bg-slate-800/60 border border-slate-800/80 hover:border-cyan-500/40 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 shadow-md"
              >
                {num}
              </button>
            ))}
            ...
            <button
              type="button"
              onClick={handleDelete}
              className="h-16 w-16 mx-auto rounded-full flex items-center justify-center text-slate-400 hover:text-rose-400 hover:bg-slate-800/60 border border-slate-800/80 hover:border-rose-500/30 active:scale-95 transition-all duration-200 focus:outline-none shadow-md"
              title="Delete digit"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="m21 4-10 0-7 8 7 8 10 0a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2Z"/><path d="m18 9-6 6"/><path d="m12 9 6 6"/></svg>
            </button>
          </div>
          ...
          <div className="mt-12 h-6 flex items-center justify-center">
            {unlocked ? (
              <div className="flex items-center text-emerald-400 text-xs tracking-widest uppercase font-medium animate-pulse">
                <Unlock className="w-3.5 h-3.5 mr-2" /> Access Granted...
              </div>
            ) : (
              <div className="flex items-center text-slate-400 text-[0.65rem] tracking-[0.2em] uppercase font-semibold">
                <Lock className="w-3 h-3 mr-2 text-cyan-400" /> Secure Passcode
              </div>
            )}
          </div>
        </div>
