...
export const BlendCalculator: React.FC<BlendCalculatorProps> = ({ onSaveAsProtocol }) => {
  ...
  const handleSaveProtocol = () => {
    ...
    onSaveAsProtocol({
      ...
      doseUnit: primaryComp?.doseUnit || 'mcg',
      syringeType,
      isBlend: true,
  ...
  return (
    <div className="flex flex-col gap-4 max-w-5xl mx-auto pb-10 w-full min-w-0 max-w-full overflow-x-hidden">
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6">
        ...
        {onSaveAsProtocol && (
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-bold text-xs sm:text-sm shadow-lg shadow-purple-500/20 transition active:scale-95 shrink-0"
          >
      ...
      <div className="lg:hidden bg-slate-900/95 border border-purple-500/40 p-3 rounded-2xl shadow-xl flex items-center justify-between gap-2 backdrop-blur-md sticky top-16 z-20">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-xl bg-purple-500/20 flex items-center justify-center text-purple-400 font-bold shrink-0 text-base">
            💉
          </div>
          <div className="min-w-0">
            <div className="text-[10px] uppercase font-bold text-slate-400 truncate">Pull in Syringe</div>
            <div className="text-base font-black text-purple-300 font-mono leading-none">
        ...
        <div className="text-right shrink-0">
          <div className="text-[10px] text-slate-400 font-medium font-mono">({result.drawVolumeMl} mL)</div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none touch-pan-x sm:flex-wrap w-full min-w-0 max-w-full">
          {PRESET_BLENDS.map((preset, idx) => (
            <button
              ...
              className="px-2.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-purple-300 hover:text-white rounded-xl text-xs font-medium border border-purple-900/50 hover:border-purple-500/50 transition shrink-0 flex items-center gap-1.5 shadow-sm active:scale-95"
            >
          ...
          <div className="glass-panel p-4 sm:p-6 rounded-2xl flex flex-col gap-4 overflow-hidden">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between gap-2">
                <label className="text-xs font-bold text-purple-300 uppercase tracking-wider truncate">
                  Peptides in this Vial ({components.length})
                </label>
                <button
                  type="button"
                  onClick={handleAddComponent}
                  className="flex items-center gap-1 text-xs font-bold text-purple-400 hover:text-purple-300 bg-purple-950/60 hover:bg-purple-900/80 px-2.5 py-1 rounded-lg border border-purple-800 transition shrink-0 active:scale-95"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline">Add Peptide ({components.length + 1})</span>
                  <span className="sm:hidden">+ Add Peptide</span>
                </button>
              </div>
              ...
              {components.map((comp, idx) => (
                <div 
                  key={comp.id}
                  className={`p-3 sm:p-3.5 rounded-xl border flex flex-col gap-2.5 transition ${
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2 w-full sm:w-auto flex-1 min-w-0">
                      <span className="shrink-0 w-6 h-6 rounded-full bg-purple-500/20 text-purple-300 font-mono font-bold text-xs flex items-center justify-center">
                        {idx + 1}
                      </span>
                      <input
                        type="text"
                    ...
                    <div className="flex items-center justify-end gap-2 shrink-0">
                      ...
                      >
                        ...
                        <span>{comp.id === primaryComponentId ? 'Target Dose' : 'Base on this'}</span>
                      </button>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <label className="block text-[10px] font-semibold text-slate-400 uppercase mb-0.5 truncate">
                        Powder (mg)
                      </label>
                      <input
                        type="number"
                        ...
                        className="w-full bg-slate-950 border border-slate-700 text-white text-xs sm:text-sm rounded-lg p-2 focus:border-purple-400 outline-none font-mono"
                      />
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-0.5">
                        <label className="text-[10px] font-semibold text-slate-400 uppercase truncate">
                          {comp.id === primaryComponentId ? 'Target Dose' : 'Delivered'}
                        </label>
                        <div className="flex bg-slate-950 rounded border border-slate-700 text-[9px] shrink-0">
                          <button
                            type="button"
                      ...
                      <input
                        ...
                        className={`w-full bg-slate-950 border text-white text-xs sm:text-sm rounded-lg p-2 outline-none font-mono ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-slate-800">
              <div>
                ...
                <div className="flex gap-1.5 mb-2 overflow-x-auto pb-1 scrollbar-none touch-pan-x">
                  {[1.0, 2.0, 2.5, 3.0, 5.0].map(v => (
                    <button
                      key={`bac-${v}`}
                      type="button"
                      onClick={() => setBacWaterMl(v)}
                      className={`px-2.5 py-1 text-xs rounded-lg font-bold transition shrink-0 active:scale-95 ${
                        bacWaterMl === v ? 'bg-purple-500 text-white shadow-sm' : 'bg-slate-800 text-slate-400 hover:text-white'
                      }`}
                    >
                      {v} mL
                    </button>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">
                Vial Cost ($ USD) <span className="text-slate-500 font-normal">(Optional)</span>
              </label>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 min-w-0">
          ...
          <div className="glass-panel p-4 sm:p-6 rounded-2xl border-purple-500/30 flex flex-col gap-3 relative overflow-hidden">
            ...
            <div className="text-xs font-semibold uppercase tracking-wider text-purple-400">
              Multi-Peptide Draw Result
            </div>

            <div className="bg-slate-950/80 border border-purple-500/30 rounded-2xl p-4 sm:p-5 text-center flex flex-col items-center justify-center gap-1 shadow-inner">
              <span className="text-[11px] text-slate-400 uppercase font-semibold">Pull in Syringe</span>
            ...
            <div className="flex flex-col gap-2 pt-2 border-t border-slate-800">
              <div className="text-[11px] font-bold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                <span>Peptides in this Dose</span>
                <span className="text-purple-400 text-[10px] font-mono">{result.totalDosesInVial} doses/vial</span>
              </div>

              <div className="flex flex-col gap-2">
                {result.components.map(comp => (
                  <div key={comp.id} className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="text-xs font-bold text-white flex items-center gap-1.5 truncate">
                        <span className="truncate">{comp.peptideName}</span>
                        {comp.id === primaryComp?.id && (
                          <span className="text-[9px] px-1.5 py-0.5 bg-purple-950 text-purple-300 border border-purple-800 rounded font-semibold shrink-0">
                            Target
                          </span>
                        )}
                      </div>
              ...
              {result.costPerDose !== undefined && (
                <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-2.5 flex items-center justify-between mt-1">
                  <span className="text-xs text-slate-400">Cost per Dose:</span>
              ...
              {onSaveAsProtocol && (
                <button
                  type="button"
                  onClick={handleSaveProtocol}
                  className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-purple-500/20 transition active:scale-95 flex items-center justify-center gap-2"
                >
                  <BookmarkPlus className="w-4 h-4" />
                  <span>Save Blend as Routine</span>
                </button>
